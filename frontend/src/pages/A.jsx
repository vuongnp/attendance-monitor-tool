...
import {processImgAntiFromCanvas} from "../utils/spoofing";
...
import ModelSpoofing from "../models/anti_MiniFASNetV1SE.onnx";
import { socket } from "../App";
import "./Attendance.css";

let inferenceSession;
let inferenceSessionVec;
let inferenceSessionAnti;
...
const loadModel = async () => {
  ...
  inferenceSessionAnti = await InferenceSession.create(ModelSpoofing);
  console.log("Model anti spoofing loaded");
    ...
    if(!finished){
      ...
      if (delta_time<TIME_TO_STOP && loading < 10){
        ...
        if (dets.length > 1) {
          ...
          drawAfterDetect("dstCanvas", dets);
          const onnxTensorAnti = await processImgAntiFromCanvas(dets[0], canvas.current, CAM_WIDTH, CAM_HEIGHT, 4);
          console.log(onnxTensorAnti);
          console.log(inferenceSessionAnti);
          console.log(inferenceSessionVec);
          console.log(inferenceSession);
          const resanti = await inferenceSessionAnti.run({ "input": onnxTensorAnti });
          console.log(resanti)
          const onnxTensorVec = await processImgVectorizeFromCanvas(
            dets[0],
            canvas.current
          );
          // console.log(onnxTensorVec);
